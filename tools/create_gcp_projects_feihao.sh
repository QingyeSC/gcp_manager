...
# 临时文件用于跟踪进度
PROGRESS_DIR="/tmp/gcp_script_feihao_$$"
...
echo "===== GCP项目批量创建脚本（非号专用版）====="
echo "项目创建数量: $PROJECT_COUNT （固定10个）"
echo "后续处理数量: $PROCESS_COUNT （固定3个）"
echo "每项目服务账号数: 1"
...
echo "⚠️  特殊说明:"
echo "  - 会创建10个项目"
echo "  - 只对前3个项目进行后续操作（绑定账单、启用API、创建服务账号等）"
echo "  - 后7个项目仅创建，不进行其他操作"
show_separator
...
for i in $(seq 0 $((PROCESS_COUNT - 1))); do
    if [ $i -lt ${#created_projects[@]} ]; then
        processing_projects+=("${created_projects[$i]}")
        processing_names+=("${project_names[$i]}")
    fi
done

echo ""
echo "🎯 选择要进行后续操作的项目（前 $PROCESS_COUNT 个）:"
for i in "${!processing_projects[@]}"; do
    echo "  $((i+1)). ${processing_names[$i]} (ID: ${processing_projects[$i]})"
done

echo ""
echo "⚠️  跳过后续操作的项目（仅创建）:"
for i in $(seq $PROCESS_COUNT $((${#created_projects[@]} - 1))); do
    if [ $i -lt ${#created_projects[@]} ]; then
        echo "  $((i+1)). ${project_names[$i]} (ID: ${created_projects[$i]})"
    fi
done
...
echo "💰 第五步：为前 $PROCESS_COUNT 个项目绑定账单"

billing_count=${#billing_accounts_array[@]}
if [ $billing_count -eq 0 ]; then
    echo "⚠️  警告: 没有可用的账单账号，跳过账单绑定"
elif [ ${#processing_projects[@]} -eq 0 ]; then
    echo "⚠️  警告: 没有可处理的项目，跳过账单绑定"
else
    # 准备账单绑定任务
    billing_jobs=()
    for i in "${!processing_projects[@]}"; do
        project_id=${processing_projects[$i]}
        # 循环使用账单账号
...
echo "🔧 第六步：为前 $PROCESS_COUNT 个项目启用必要的API服务（并发处理）"

if [ ${#processing_projects[@]} -eq 0 ]; then
    echo "⚠️  警告: 没有可处理的项目，跳过API启用"
else
    # 准备API启用任务
    api_jobs=()
    for project_id in "${processing_projects[@]}"; do
...
#####################################################
#     第七步：为前3个项目创建服务账号并授予Vertex AI权限 #
#####################################################

echo "👤 第七步：为前 $PROCESS_COUNT 个项目创建服务账号并授予权限（并发处理）"

if [ ${#processing_projects[@]} -eq 0 ]; then
    echo "⚠️  警告: 没有可处理的项目，跳过服务账号创建"
else
    # 准备服务账号创建任务
    sa_jobs=()
    for project_id in "${processing_projects[@]}"; do
...
#####################################################
#       第八步：为前3个项目下载服务账号密钥并上传       #
#####################################################

echo "🔑 第八步：为前 $PROCESS_COUNT 个项目下载服务账号密钥并上传到管理系统（并发处理）"

if [ ${#processing_projects[@]} -eq 0 ]; then
    echo "⚠️  警告: 没有可处理的项目，跳过密钥处理"
else
    # 准备密钥处理任务
    key_jobs=()
    for i in "${!processing_projects[@]}"; do
        project_id=${processing_projects[$i]}
        project_name=${processing_names[$i]}
...
echo "🎯 特殊配置: 创建 $PROJECT_COUNT 个项目，处理前 $PROCESS_COUNT 个"
echo ""
echo "💳 使用的账单账号:"
...
echo ""
echo "📁 项目处理结果:"
echo "  - 总共创建: ${#created_projects[@]} 个项目"
echo "  - 完整处理: ${#processing_projects[@]} 个项目（前$PROCESS_COUNT个）"
echo "  - 仅创建不处理: $((${#created_projects[@]} - ${#processing_projects[@]})) 个项目"
echo ""
...
if [ ${#created_projects[@]} -gt 0 ]; then
    ...
    for i in "${!created_projects[@]}"; do
        ...
        if [ $i -lt $PROCESS_COUNT ]; then
            billing_index=$((i % ${#billing_accounts_array[@]}))
            if [ ${#billing_accounts_array[@]} -gt 0 ]; then
                billing_name=${billing_names_array[$billing_index]}
                echo "    $((i+1)). $project_name (ID: $project_id) ✅ 完整处理（绑定账单: $billing_name）"
            else
                echo "    $((i+1)). $project_name (ID: $project_id) ✅ 完整处理（未绑定账单）"
            fi
        else
            echo "    $((i+1)). $project_name (ID: $project_id) ⚪ 仅创建"
        fi
    done
fi
...
echo ""
echo "👤 服务账号创建结果:"
echo "  - 每个处理项目创建: 1 个服务账号"
echo "  - 预期创建总数: ${#processing_projects[@]} 个"
if [ -f "$PROGRESS_DIR/service_accounts" ]; then
...
echo "  - 实际成功创建: ${#created_projects[@]}"
echo "  - 项目创建成功率: $(( ${#created_projects[@]} * 100 / PROJECT_COUNT ))%"
echo "  - 处理项目数量: ${#processing_projects[@]}/${#created_projects[@]}"

if [ -f "$PROGRESS_DIR/enabled_apis" ]; then
    total_apis=$(( ${#processing_projects[@]} * ${#APIS_TO_ENABLE[@]} ))
...
echo ""
echo "✅ 所有操作完成！"
echo "   ✓ 已创建 ${#created_projects[@]} 个项目"
echo "   ✓ 前 ${#processing_projects[@]} 个项目已完成完整配置："
echo "     - 绑定账单"
echo "     - 启用API服务"
echo "     - 创建服务账号并授予Vertex AI权限"
if [ ! -z "$UPLOAD_API_URL" ]; then
    echo "     - 下载并上传密钥文件到管理系统"
else
    echo "     - 下载密钥文件到本地"
fi
echo "   ✓ 后 $((${#created_projects[@]} - ${#processing_projects[@]})) 个项目仅创建完成，未进行其他配置"
echo ""
echo "💡 使用说明："
echo "  - 项目ID: 由GCP自动生成的随机唯一标识符"
echo "  - 项目名称: feihao-{邮箱前缀}-special-{序号}"
echo "  - 密钥文件命名格式：项目名称.json（仅前${PROCESS_COUNT}个项目）"
if [ ! -z "$UPLOAD_API_URL" ]; then
