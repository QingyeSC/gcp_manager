...
link_billing_for_gemini_projects() {
    ...
    if [ -f "$CREATED_PROJECTS_FILE" ]; then
        mapfile -t GEMINI_PROJECTS < "$CREATED_PROJECTS_FILE"
        log_info "从创建记录中读取到 ${#GEMINI_PROJECTS[@]} 个项目"
    else
        # 如果没有记录文件，等待3秒后再查询
        log_info "等待3秒以确保项目在API中可见..."
        sleep 3
        mapfile -t GEMINI_PROJECTS < <(gcloud projects list --filter="projectId:gemini-project-${TIMESTAMP}-*" --format="value(projectId)" 2>/dev/null)
    fi
...
if [ -f "$BILLING_SUCCESS_FILE" ]; then
    ...
    log_info "共有 ${#READY_FOR_KEYS[@]} 个项目成功绑定账单，将为它们创建 API Key"
else
...
# 合并所有临时key文件到key.txt
log_info "合并所有已生成的 API Keys 到 key.txt..."
